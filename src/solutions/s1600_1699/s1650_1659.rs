...
impl Solution {
    ...
    pub fn minimum_jumps(forbidden: Vec<i32>, a: i32, b: i32, x: i32) -> i32 {
        ...
        let mut queue = vec![JumpInfo{position: 0, direction: 1, step: 0}];
        let lower_bound = 0;
        let upper_bound = if a == b {
            x
        } else if a > b {
            x + b
        } else {
            x.max(forbidden.iter().max().unwrap() + a) + b
        };
        ...
        while let Some(jump_info) = queue.pop() {
            ...
            if next_position <= upper_bound && !visited_position.contains(&next_position) && !forbidden.contains(&next_position) {
                visited_position.insert(next_position);
                queue.insert(0, JumpInfo { position: next_position, direction: 1, step: jump_info.step + 1 });
            }
            if jump_info.direction > 0 {
                let next_position = jump_info.position - b;
                if next_position > lower_bound && !visited_position.contains(&(-next_position)) && !forbidden.contains(&next_position) {
                    visited_position.insert(-next_position);
                    queue.insert(0, JumpInfo { position: next_position, direction: -1, step: jump_info.step + 1 });
                }
            }
